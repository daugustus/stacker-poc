...
class AutoscalingGroup(Blueprint):
    ...
    def get_launch_configuration_userdata(self):
        return Base64(Join('', [
            ...
            "/usr/bin/cloud-init-per once bootstrap-deps1 yum install cloud-utils python-pip jq -y\n",
            "/usr/bin/cloud-init-per once bootstrap-deps2 pip install awscli botocore boto\n",
            "/usr/bin/cloud-init-per always bootstrap-pull aws s3 cp s3://",
            Ref("BootstrapBucket"),
            "/configs/bootstrap.sh /root/bootstrap.sh\n",
